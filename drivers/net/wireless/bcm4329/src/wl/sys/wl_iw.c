...
#ifdef CONFIG_ARC_WIFI_PM
#include <linux/workqueue.h>
#include <linux/sched.h>

extern int ARC_wifi_dbm;
extern bool ARC_wifi_wake_pm;

extern void ARC_wifi_register_dbm_cb(void*);
#endif
...
void ARC_wifi_set_dbm_value(int dbm) {
...
/*
 ...
 printk("%s: current dbm value is %d, ARC_wifi_dbm is %d\n", __FUNCTION__, curr_dbm, ARC_wifi_dbm);
 
 mutex_lock(&dbm_mutex);
 
 if (ARC_wifi_dbm != curr_dbm) {
 if (dev_wlc_intvar_set(dev, "qtxpower", bcm_qdbm_to_mw((ARC_wifi_dbm * 4)-0.5))) {
 printk("%s: cannot set txpower !\n", __FUNCTION__);
 } else {
 check->dbm = ARC_wifi_dbm;
 }
 }
...
static int ARC_wifi_dbm = DEFAULT_DBM;
#endif
...
/* LGE_CHANGE_E, [yoohoo@lge.com], 2009-11-19, Use deepsleep instead of dhd_dev_reset when driver start or stop */
	...
	if (curr_dev != dev) {
		...
		printk("%s: registering ARC callbacks\n", __FUNCTION__);

		ARC_wifi_set_dbm_value(ARC_wifi_dbm);
		ARC_wifi_register_dbm_cb(ARC_wifi_set_dbm_value);
	}
#endif //CONFIG_ARC_WIFI_PM
...
#ifdef CONFIG_ARC_WIFI_PM
	ARC_wifi_dbm = (int)(bcm_mw_to_qdbm(txpwrmw) / 4);
	ARC_wifi_set_dbm_value(ARC_wifi_dbm);
#endif
